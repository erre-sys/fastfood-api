...
public class JPABaseRepository<T, ID extends Serializable> {
	...
	protected BooleanExpression getPredicate(String key, String operator, String value, PathBuilder<?> entityPath,
		...
		case "BigDecimal":
			if (isMultiValue) {
				...
				NumberPath<BigDecimal> path = entityPath.getNumber(key, BigDecimal.class);
				BigDecimal[] numValue = new BigDecimal[1];
				numValue[0] = BigDecimal.valueOf(Double.parseDouble(value));
				return getNumberPredicate(path, operator, numValue);
			}
		case "Double":
			if (isMultiValue) {
				...
				NumberPath<Double> path = entityPath.getNumber(key, Double.class);
				Double[] numValue = new Double[1];
				numValue[0] = Double.parseDouble(value);
				return getNumberPredicate(path, operator, numValue);
			}
		case "Boolean":
			if (":".equals(operator)) {
	...
	protected OrderSpecifier<?> getOrderSpecifier(PathBuilder<?> pathBuilder, CriterioOrden criterioOrden,
			Class<?> classType) {
	...
	private <T extends Number & Comparable<?>> BooleanExpression getNumberPredicate(NumberPath<T> path, String operator,
			T[] value) {
		switch (operator) {
		case "=":
